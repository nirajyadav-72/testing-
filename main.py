...
# ⏳ [MANDATORY] एक्टिव बैन काउंटडाउन ट्रैकर्स के लिए डिक्शनरी
active_ban_timers = {}
...
# =====================================================================
# 👑 3.5 /promote कमांड हैंडलर (Reply, Username, ya Name se)
# =====================================================================
...
def handle_promote_command(message):
    ...
    if SUPPORT_GROUP_ID is None or message.chat.id != SUPPORT_GROUP_ID:
        try:
            bot.reply_to(message, "❌ यह कमांड केवल मुख्य सपोर्ट组 के अंदर ही इस्तेमाल की जा सकती है!")
        except Exception:
            pass
        return
    ...
    # 1. रास्ता 1: अगर किसी के मैसेज पर रिप्लाई किया गया है
    if message.reply_to_message:
        user_id_to_promote = message.reply_to_message.from_user.id
        user_name = message.reply_to_message.from_user.first_name

    # 2. रास्ता 2: अगर कमांड के आगे टेक्स्ट (Username, ID, या Name) लिखा है
    else:
        args = message.text.split(maxsplit=1)
        if len(args) > 1:
            input_text = args[1].strip()
            
            # (A) चेक करें कि क्या डायरेक्ट संख्या (User ID) दी गई है
            if input_text.isdigit():
                user_id_to_promote = int(input_text)
            
            # (B) चेक करें कि क्या @username दिया गया है या नॉर्मल नाम दिया गया है
            else:
                clean_search = input_text.replace("@", "").lower()
                try:
                    with sqlite3.connect(DB_FILE, timeout=20) as conn:
                        cursor = conn.cursor()
                        # डेटाबेस में 'users' टेबल से नाम या यूजरनेम मैच करके ID निकालना
                        cursor.execute(
                            "SELECT user_id, user_name FROM users WHERE LOWER(user_name) LIKE ? OR LOWER(user_name) = ? LIMIT 1",
                            (f"%{clean_search}%", clean_search)
                        )
                        row = cursor.fetchone()
                        if row:
    ...
    # अगर इनपुट देने के बाद भी यूज़र डेटाबेस में या रिप्लाई में नहीं मिला
    if not user_id_to_promote:
        try:
            error_msg = (
                "💡 <b>तरीका:</b> यूज़र के मैसेज पर रिप्लाई करके <code>/promote</code> लिखें,\n"
                "या फिर <code>/promote @username</code> या <code>/promote exact_name</code> लिखें।\n\n"
                "⚠️ <i>नोट: अगर आप नाम से प्रमोट कर रहे हैं, तो उस यूज़र का बॉट के डेटाबेस में होना (ग्रुप में एक्टिव होना) ज़रूरी है!</i>"
            )
    ...
    try:
        # बॉट के द्वारा यूज़र को एडमिन राइट्स देना
        bot.promote_chat_member(
            ...
            can_delete_messages=True,    # मैसेज डिलीट करने की पावर
            can_invite_users=True,       # इनवाइट करने की पावर
            can_restrict_members=True,   # म्यूट/बैन करने की पावर
            can_pin_messages=True,       # पिन करने की पावर
            can_promote_members=False,   # नए एडमिन बनाने की पावर (बॉट इसे अपने पास रखेगा)
        ...
        success_text = (
            ...
            f"💡 <i>अब ओनर सर के आदेश पर यह बॉट इस एडमिन को डिमोट या बैन भी कर सकता है।</i>"
        )
        try:
        ...
        try:
            bot.reply_to(message, f"❌ प्रमोट करने में विफलता आई: {e}\n\n<i>नोट: कृपया पक्का करें कि बॉट खुद ग्रुप में Admin है और उसके पास 'Add New Admins' की Permission ऑन है!</i>")
        except Exception:
            pass
